...
baseline_size=reduce((p, q) -> count[p] >= count[q] ? p : q, keys(count))
println("Baseline Content Size: ", baseline_size )
println("Number of occurence in bids ", max_count )
######################################################################################################################################################
# Determine the payment  that minimize the total payment with GurobiSolver and with winner with winner
R=bids
i = 1;
...
p =plot(z,t,color="red",marker="o",markersize=10, linewidth=2.0, label="Optimal: ISP Profit")
ax = gca()
title("In-Network Caching Profits", fontsize=20)
grid("on")
xlabel("Number of customers", fontsize=20)
font1 = Dict("color"=>"red")
ylabel("Opimal ISP Profit",fontdict=font1, fontsize=20)
...
ylabel("ISP Profit",fontdict=font2, fontsize=20)
p =plot(z,u,color="purple", marker="*", markersize=20, linewidth=2.0, label="VCG: ISP Profit")# Plot a basic line
...
ax3[:spines]["top"][:set_visible](true) # Hide the top edge of the axis
