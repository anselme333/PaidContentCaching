...
distributor_price = rand(5:10,50) # Vector of bids in the bundle
content_size = rand(5:500,50) # Vector of content size received in the bundle in terms of megabyte
...
# Determine the payment  that minimize the total payment with standard VCG
		 ...
		 #global optimum_price= abs(tatal_evaluation_withoutwinner-tatal_evaluation_withwinner)
...
# Determine the profit of providers with variable number of customer [1000,10000]
function provider_profits() # provider_profits
number_router=200 # Number of the routers inside the ISP
total_cache_size = cache_size * number_router #In  terms of megabyte
# Determine the profit of content povider and Internet Service providers
numbercostomer=1000 #Number of customer Interested in paid content

t=[] # ICSP profit array Initiliazation
z=[] # Number of customer array Initiliazation
u=[] # ICSP profit0 array Initiliazation with
v=[] # CP profit array Initiliazation

while numbercostomer<=10000

	cache_cost =(cache_price * ( total_cache_size))/36 # the price of caching per each month, and cahe get replaced every 3 years(36 months)
	...
	#ISP profits due to in network caching
	isp_profit = p1-p2_normal-cache_cost
	isp_profit0 = p10-p2_normal-cache_cost

	#CP profits due to in network caching
	cp=(numbercostomer * (optimum_price  * 1)) + (0.70 * ((numbercostomer-1) * (optimum_price *((monthly_volume_demand/baseline_size)-1))))# only cp
	cp0=(numbercostomer * (optimum_price0  * 1)) + (0.70 * ((numbercostomer-1) * (optimum_price0 *((monthly_volume_demand/baseline_size)-1))))# only cp

	#ICSP profits due to in network caching
	cache_cost_ICSP= (monthly_volume_WithCache * cache_price)/36
	ICSP_profit =p2-cache_cost_ICSP
	ICSP_profit0 =p20-cache_cost_ICSP

println("numbercostomer :", numbercostomer)
println("total cache size:", total_cache_size)
println("cache_cost:", cache_cost)
println("isp_profit:", isp_profit)
println("P1:", p1)
println("p2: ", p2)
println("monthly_volume without caching :",monthly_volume_Without_Cache)
println("monthly_volume with caching :",monthly_volume_WithCache)

z=append!(z, numbercostomer)
t=append!(t, ICSP_profit)
u=append!(u, ICSP_profit0)
v=append!(v, cp)

numbercostomer=numbercostomer + 1000
end
##########################################################################################################################################

# In-Network Caching ICSP rofits graph
fig = figure("pyplot_multiaxis",figsize=(10,10))
p =plot(z,t,color="red",marker="o", linewidth=2.0, markersize=10, label="Optimal: ICSP Profit")
ax = gca()
title("In-Network Caching Profits", fontsize=20)
grid("on")
xlabel("Number of customers", fontsize=20)
font1 = Dict("color"=>"red")
ylabel("Optimal ICSP Profit",fontdict=font1, fontsize=20)
...
ylabel("ICSP Profit",fontdict=font2, fontsize=20)
p =plot(z,u,color="purple",marker="*", linewidth=2.0, markersize=20, label="VCG: ICSP Profit")# Plot a basic line
...
ax3[:spines]["right"][:set_position](("axes",1.10)) # Offset the y-axis label from the axis itself so it doesn't overlap the second axis
font3 = Dict("color"=>"green")
ylabel("CP profit",fontdict=font3, fontsize=20)
p = plot_date(z,v, color="green",linestyle="-",linewidth=2.0, marker="x", markersize=20, label="VCG: CP profit") # Plot a basic line
