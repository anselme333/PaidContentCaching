...
while numbercostomer<=10000
	...
	global monthly_volume_demand = ((baseline_size ) * 30) * numbercostomer # We assume that each customer request one file per day
...
w=append!(w,total_cache_size )
q=append!(q, monthly_volume_WithCache)
s=append!(s, monthly_volume_demand  )
numbercostomer=numbercostomer + 1000
end

##########################################################################################################################################
# Monthly Traffic Volume graph
fig = figure("pyplot_multiaxis",figsize=(10,10))
p =plot(z,w, color="red",marker="o", linewidth=2.0, markersize=10, label="Total Cache Size")
ax = gca()
title("Monthly Traffic Volume (MTV)", fontsize=20)
grid("on")
xlabel("Number of customers", fontsize=20)
font1 = Dict("color"=>"red")
ylabel("Total Cache Size",fontdict=font1, fontsize=20)
...
p =plot(z,q, color="purple",marker="*", linewidth=2.0, markersize=20, label="MTV (caching)")# Plot a basic line
ylabel("MTV (caching)",fontdict=font2, fontsize=20)
...
p = plot_date(z,s, color="green",linestyle="-",marker="x", linewidth=2.0, markersize=20,label="MTV ( No caching)") # Plot a basic line
ylabel("MTV ( No caching)",fontdict=font3, fontsize=20)
