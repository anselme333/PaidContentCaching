...
# Determine the payment  that minimize the total payment with GurobiSolver  with winner
# With winner
...
##########################################################################################################################################
# Changing the number of the routers
cache_size =100000 # Number of the router inside the ISP
...
while cache_size<=1000000
total_cache_size = cache_size * number_router
...
println("numbercostomer :", number_router)
...
grid("on")
xlabel("Cache size (MB)", fontsize=20)
...
p =plot(z,u,color="purple",marker="*",markersize=20, linewidth=2.0, label="ICSP Profit")# Plot a basic line
...
ax3[:spines]["right"][:set_position](("axes",1.14)) # Offset the y-axis label from the axis itself so it doesn't overlap the second axis
font3 = Dict("color"=>"green")
ylabel("CP profit",fontdict=font3, fontsize=20)
p = plot_date(z,v, color="green",linestyle="-",linewidth=2.0, marker="x",markersize=20,label="CP profit") # Plot a basic line
