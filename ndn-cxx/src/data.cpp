...
Data::wireEncode(EncodingImpl<TAG>& encoder, bool unsignedPortion/* = false*/) const
{
  ...
  // price
   //totalLength += encoder.prependBlock(getPrice());
 ...
 //uint8_t content_price[8];

  if (!unsignedPortion)
    {
      totalLength += encoder.prependVarNumber(totalLength);
      totalLength += encoder.prependVarNumber(tlv::Data);
      //totalLength += encoder.prependByteArrayBlock(tlv::Data, content_price,8);
    }
...
Data::wireDecode(const Block& wire)
{
  ...
  // price
  //content_price = m_wire.get(tlv::ContentPrice);
...
Data::getPrice() const
{
  if (content_price.empty())
	  content_price = makeEmptyBlock(tlv::ContentPrice);
  if (!content_price.hasWire())
	  content_price.encode();
...
Data&
Data::setPrice(const Block& contentp)
{
  onChanged();

  if (contentp.type() == tlv::ContentPrice)
	content_price= contentp;
  else {
	content_price = Block(tlv::ContentPrice, contentp);
  }
  price_item = 45;// Content Price in USD
  return *this;
}
...
operator<<(std::ostream& os, const Data& data)
{
  ...
  os << "ContentPrice: (size: " << data.getPrice().value_size() << ")\n";
